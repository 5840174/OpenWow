...
void EnvironmentManager::outdoorLighting()
{
	...
	//m_OutdoorDayDiffuseColor = vec4(m_SkyManager->GetColor(ESkyColors::SKY_COLOR_GLOBAL_DIFFUSE) * dayNightPhase.dayIntensity, 1.0f);
	//m_OutdoorNightDiffuseColor = vec4(m_SkyManager->GetColor(ESkyColors::SKY_COLOR_GLOBAL_DIFFUSE) * dayNightPhase.nightIntensity, 1.0f);
